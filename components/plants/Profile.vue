...
import { PublicUser } from '../../types/user/PublicUser';


const props = defineProps<{
    seeConversation: boolean,
    plant: Plant,
}>()
...
const plantOwner = ref<PublicUser | null>(null);
(async () => {
    const user = await fetch("http://localhost:8080/user/" + props.plant.userId);
    plantOwner.value = await user.json();
})
</script>
        ...
        <div class="flex overflow-auto flex-1">
            <div class="flex flex-1 flex-col p-3 overflow-auto">
                <div class="flex w-full items-center flex-col ">
                    ...
                    <h1 class="text-accent font-bold text-3xl">{{ plant.name }}</h1>

                    <div class="flex flex-wrap space-x-5 mt-5 select-none items-center justify-center">
                        <p class="m-1">Taille : <span class="badge badge-lg">{{ plant.size }}cm</span></p>
                        <p class="m-1">Age : <span class="badge badge-lg">Bonne question</span></p>
                        <p class="m-1" v-if="plantOwner">Propriétair : <NuxtLink :to="`/home/user/${plantOwner?.id}`">
                                <span class="badge badge-lg text-accent">{{ plantOwner?.firstname + " " +
                                    plantOwner?.lastname }}</span>
                            </NuxtLink>
                        </p>
                    </div>
                </div>
                    ...
                    <div class="form-control flex-1 max-h-full min-w-[500px]">
                        <label class="label">
                            <span class="label-text text-accent font-bold text-xl">Description de la
                                plante</span>
                            <button class="btn btn-xs" @click="handleDescriptionModification">{{
                                isModifyinDescription ? "Envoyer" :
                                "Modifier" }}</button>

                        </label>
                        <textarea class="textarea textarea-bordered h-full disabled:bg-base-300"
                            :disabled="!isModifyinDescription" placeholder="Bio" v-model="plant.description"></textarea>
                    </div>
                ...
                <div class="flex  flex-wrap space-x-2">
                    ...
                    <div class="form-control flex-1 ">
                        <label class=" label">
                            <span class="label-text text-accent font-bold text-xl">Créneaux disponible </span>
                            <h1 class="font-semibold btn btn-xs">Ajouter un créneau</h1>
                        </label>
                        <div class="flex rounded-2xl bg-base-100 p-1 h-full  ">
